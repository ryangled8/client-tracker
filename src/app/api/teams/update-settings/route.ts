...
export async function PUT(req: Request) {
  try {
    ...
    const { teamId, name, settings, coaches, removeCoachIds } = await req.json()
    ...
    if (settings) {
      ...
      if (
        ...
        (settings.noticePeriodWeeks < 1 || settings.noticePeriodWeeks > 12)
      ) {
        return NextResponse.json(
          { error: "Notice period must be between 1 and 12 weeks" },
          { status: 400 }
        )
      }
      ...
      if (
        settings.dateFormat &&
        !["dd/mm/yyyy", "mm/dd/yyyy"].includes(settings.dateFormat)
      ) {
        return NextResponse.json({ error: "Invalid date format" }, { status: 400 })
      }
      updateData.settings = settings
    ...
    if (coaches) {
      let updatedCoaches = team.coaches
    
      if (Array.isArray(removeCoachIds) && removeCoachIds.length > 0) {
        updatedCoaches = updatedCoaches.filter(
          (coach) => !removeCoachIds.includes(coach._id.toString())
        )
      }
    
      updatedCoaches = updatedCoaches.map((coach) => {
        const updatedCoach = coaches.find(
          (c: any) => c._id === coach._id.toString()
        )
        if (updatedCoach && updatedCoach.coachColor) {
          coach.coachColor = updatedCoach.coachColor
        }
        return coach
      })
    
      updateData.coaches = updatedCoaches
    }
